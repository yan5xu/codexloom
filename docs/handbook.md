...
- `Session`：持久化元数据，包含 `id/name/cwd/threadId/sandbox/approvalPolicy/model/status/currentTask/currentTurnId/lastError/lastTurn/source`。
...
6. hub 启动 watchdog。
7. 调 `turn/start`，请求返回后写 `hub/turn-started`。
