...
func TestPassiveHTTPServeShutdownLeavesEntireDurableTreeUnchanged(t *testing.T) {
	dir := t.TempDir()
	...
	before := snapshotPassiveHTTPTree(t, dir)
	...
	if response.Code != http.StatusOK {
		t.Fatalf("passive HTTP read = %d: %s", response.Code, response.Body.String())
	}
	h.Shutdown()
	if err := st.Close(); err != nil {
		t.Fatal(err)
	}
	after := snapshotPassiveHTTPTree(t, dir)
	if len(before) != len(after) {
		t.Fatalf("passive HTTP fileset changed: before=%v after=%v", before, after)
	}
	for path, want := range before {
		if after[path] != want {
			t.Fatalf("passive HTTP bytes changed for %s", path)
		}
	}
	for _, unexpected := range []string{"runtime-foundation.json", "credentials", "events"} {
		if _, err := os.Stat(filepath.Join(dir, unexpected)); !os.IsNotExist(err) {
			t.Fatalf("passive HTTP created %s: %v", unexpected, err)
		}
	}
}

func snapshotPassiveHTTPTree(t *testing.T, root string) map[string][sha256.Size]byte {
	t.Helper()
