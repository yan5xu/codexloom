...
func (h *Hub) StageThreadArtifact(key, name, declaredMime string, source io.Reader) (ThreadArtifact, error) {
	...
	var staged ThreadArtifact
	err := h.st.WithStableWriteRoot(func(root *os.Root) error {
		artifactRoot := threadArtifactRelativeRoot(agentID)
		if err := root.MkdirAll(artifactRoot, 0o700); err != nil {
			return errf(500, "create Thread artifact store: %s", err)
		}
		temporaryPath := filepath.Join(artifactRoot, "."+newIntegrationID("upload")+".tmp")
		temporary, err := root.OpenFile(temporaryPath, os.O_CREATE|os.O_EXCL|os.O_RDWR, 0o600)
		if err != nil {
			return errf(500, "stage Thread artifact: %s", err)
		}
		committed := false
		defer func() {
			_ = temporary.Close()
		...
		if copyErr != nil || syncErr != nil || closeErr != nil {
			return errf(400, "read Thread artifact")
		}
		if written <= 0 || written > MaxThreadArtifactBytes {
			return errf(400, "Thread artifact must be between 1 byte and 25 MB")
		}
		digest := hex.EncodeToString(hash.Sum(nil))
		id := "art_" + digest[:24]
		if artifact, loadErr := h.loadThreadArtifactFromRoot(root, agentID, id); loadErr == nil {
			staged = artifact
			return nil
		}

		directory := filepath.Join(artifactRoot, id)
		if err := root.MkdirAll(directory, 0o700); err != nil {
			return errf(500, "create Thread artifact directory: %s", err)
		}
		extension := safeArtifactExtension(name)
		fileName := "content" + extension
		contentPath := filepath.Join(directory, fileName)
		if err := root.Rename(temporaryPath, contentPath); err != nil {
			if _, statErr := root.Stat(contentPath); statErr != nil {
				return errf(500, "publish Thread artifact: %s", err)
			}
		}
		committed = true

		mimeType := detectArtifactMimeRoot(root, contentPath, name, declaredMime)
		...
		if err := writeArtifactRecordRoot(root, filepath.Join(directory, "metadata.json"), record); err != nil {
			return errf(500, "persist Thread artifact: %s", err)
		}
		if err := secureThreadArtifactRoot(root, directory, contentPath); err != nil {
			return errf(500, "secure Thread artifact snapshot: %s", err)
		}
		staged = h.artifactFromRecord(record)
		return nil
	})
	if err != nil {
...
func (h *Hub) loadThreadArtifactFromRoot(root *os.Root, agentID, artifactID string) (ThreadArtifact, error) {
	if root == nil || !safeStoreComponent(agentID) || !safeStoreComponent(artifactID) || !strings.HasPrefix(artifactID, "art_") {
		return ThreadArtifact{}, fmt.Errorf("invalid artifact identity")
	}
	directory := filepath.Join(threadArtifactRelativeRoot(agentID), artifactID)
	data, err := root.ReadFile(filepath.Join(directory, "metadata.json"))
	if err != nil {
		return ThreadArtifact{}, err
	}
	var record threadArtifactRecord
	...
	contentPath := filepath.Join(directory, record.FileName)
	info, err := root.Lstat(contentPath)
	if err != nil || !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Size() != record.Size {
		return ThreadArtifact{}, fmt.Errorf("artifact content is unavailable")
	}
	file, err := root.Open(contentPath)
	if err != nil {
		return ThreadArtifact{}, fmt.Errorf("artifact content is unavailable")
	}
	hash := sha256.New()
...
func (h *Hub) markThreadArtifactPublished(agentID, artifactID string) (ThreadArtifact, bool, error) {
	var artifact ThreadArtifact
	newlyPublished := false
	err := h.st.WithStableWriteRoot(func(root *os.Root) error {
		loaded, err := h.loadThreadArtifactFromRoot(root, agentID, artifactID)
		if err != nil {
			return err
		}
		artifact = loaded
		...
		if err := writeArtifactRecordRoot(root, metadataPath, record); err != nil {
			return err
		}
		if err := secureThreadArtifactRoot(root, directory, filepath.Join(directory, record.FileName)); err != nil {
			return err
		}
		artifact = h.artifactFromRecord(record)
		newlyPublished = true
	...
	return artifact, newlyPublished, err
}
...
func threadArtifactRelativeRoot(agentID string) string {
	return filepath.Join("attachments", "threads", agentID)
}
...
func detectArtifactMimeRoot(root *os.Root, path, name, declared string) string {
...
func writeArtifactRecordRoot(root *os.Root, path string, record threadArtifactRecord) error {
	data, err := json.MarshalIndent(record, "", "  ")
	if err != nil {
		return err
	}
	temporary := path + "." + newIntegrationID("metadata") + ".tmp"
...
func secureThreadArtifactRoot(root *os.Root, directory, contentPath string) error {
	if err := root.Chmod(contentPath, 0o400); err != nil {
		return err
	}
	if err := root.Chmod(filepath.Join(directory, "metadata.json"), 0o400); err != nil {
		return err
	}
	return root.Chmod(directory, 0o700)
}
